# -*- coding: utf-8 -*-
"""
Created on Thu Feb 21 13:47:48 2019

@author: Prof.D Mukhopadhyay
"""

import csv
import math
import numpy as np
import matplotlib.pyplot as plt
import sys
import os

wstart = 10
wstop = 1999

wlen = wstop-wstart

InvSbox = (
    ,
)

myfile = "_DATA1_keyset_9_attack.csv"

# Byte Hamming Weight Table
hamming_weight_8bit_table = np.array([
0,1,1,2,1,2,2,3,1,2,2,3,2,3,3,4,
1,2,2,3,2,3,3,4,2,3,3,4,3,4,4,5,
1,2,2,3,2,3,3,4,2,3,3,4,3,4,4,5,
2,3,3,4,3,4,4,5,3,4,4,5,4,5,5,6,
1,2,2,3,2,3,3,4,2,3,3,4,3,4,4,5,
2,3,3,4,3,4,4,5,3,4,4,5,4,5,5,6,
2,3,3,4,3,4,4,5,3,4,4,5,4,5,5,6,
3,4,4,5,4,5,5,6,4,5,5,6,5,6,6,7,
1,2,2,3,2,3,3,4,2,3,3,4,3,4,4,5,
2,3,3,4,3,4,4,5,3,4,4,5,4,5,5,6,
2,3,3,4,3,4,4,5,3,4,4,5,4,5,5,6,
3,4,4,5,4,5,5,6,4,5,5,6,5,6,6,7,
2,3,3,4,3,4,4,5,3,4,4,5,4,5,5,6,
3,4,4,5,4,5,5,6,4,5,5,6,5,6,6,7,
3,4,4,5,4,5,5,6,4,5,5,6,5,6,6,7,
4,5,5,6,5,6,6,7,5,6,6,7,6,7,7,8
], np.uint8)

MASK = (2**8 - 1)
samples = np.arange(wstart, wstop)
number_of_samples = len(samples)
correlation_arr = np.zeros((256,number_of_samples),dtype='float')
number_of_traces_list = [int(sys.argv[1])]
print("Number of traces: " + str(number_of_traces_list))

print("Chosen samples: " + str(samples))
print("")

childs = 0
while True:
    child_pid = os.fork()
    if child_pid == 0:
        childs += 1
        if childs == 8:
            break
    else:
        break


res = r""
for number_of_traces in number_of_traces_list:
    if childs > 0:
        for kb in range(256 - (32*childs),256 - (32*childs) + 32,1):
                res += "Processing: " + hex(kb) + "\n"
                
                H = np.zeros(number_of_traces)
                W = np.zeros([number_of_traces, number_of_samples])
                correlation = np.zeros(number_of_samples)
                with open(myfile) as csv_file:
                    csv_reader = csv.reader(csv_file, delimiter=',')
                    for idx, row in enumerate(csv_reader):
                        if idx==number_of_traces: break
                        ct = int(row[1],16)
                        ct_temp = (ct >> 0) & MASK
                        
                        r9_temp = InvSbox[ct_temp ^ kb]
                        
                        H[idx] = hamming_weight_8bit_table[r9_temp]
                        W[idx][:] = np.array([int(i) for i in np.array(row)[samples].tolist()])
                        
                for jdx in range(number_of_samples):
                    correlation[jdx] = np.corrcoef(W[:,jdx], H)[0][1]
                
                correlation_arr[kb] = np.abs(correlation)
                res += str(np.max(correlation_arr[kb])) + "\n"
                
    
    if child_pid != 0:
        os.waitpid(child_pid, 0)
    
    sys.stdout.write(res)